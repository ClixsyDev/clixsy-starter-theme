            ...
            <div class="flex pt-3">
                <?php if ($socials) { ?>
                    <?php foreach ($socials as $social_item) { ?>
                        <a href="<?php echo esc_url($social_item['link']); ?>"><?php echo wp_get_attachment_image($social_item['icon'], 'full', "", ["class" => "w-16/24"]); ?></a>
                    <?php } ?>
                <?php } ?>
            </div>
            <?php if ($subscribe_form) { ?>
                <div class="xl:ml-16 sm:w-full sm:ml-0">
                    <h5 class="text-kennyGreen font-avenir font-bold text-sm">GET OUR NEWSLETTER</h5>
                    <?php 
        ...
        <div class="flex pt-10 gap-8 xl:gap-5 mdt:flex-col lg:pt-8 lg:gap-8">
            ...
            <div class="flex gap-80 sm:flex-col sm:gap-8 xl:gap-28 mdt:justify-between">
                <div class="flex flex-col gap-6">
                    <?php if ($location) { ?>
                        <div>
                            <h5 class="text-kennyGreen font-avenir font-bold text-sm">LOCATION</h5>
                            <p class="text-white font-avenir font-bold text-2xl xl:text-xl"><?php echo $location ?></p>
                        </div>
                    <?php } ?>
                    <?php if ($phone) { ?>
                        <div>
                            <h5 class="text-kennyGreen font-avenir font-bold text-sm">EMAIL</h5>
                            <p class="text-white font-avenir font-bold text-lg xl:text-xl"><?php echo $phone ?></p>
                        </div>
                    <?php } ?>
                    <?php if ($mail) { ?>
                        <div>
                            <h5 class="text-kennyGreen font-avenir font-bold text-sm">PHONE</h5>
                            <p class="text-white font-avenir font-bold text-lg xl:text-xl"><?php echo $mail ?></p>
                        </div>
                    <?php } ?>
                </div>
                <div>
                    <h5 class="text-kennyGreen font-avenir font-bold text-sm uppercase">Quick Links</h5>
                    <?php wp_nav_menu([
                        'menu'                 => 'Footer menu',
                        'container'            => '',
                        'container_class'      => '',
