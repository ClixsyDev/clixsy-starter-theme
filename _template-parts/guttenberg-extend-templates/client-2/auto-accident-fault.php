    ...
    <section class="pt-20 xl:pt-10 mdt:mb-10">
        <div class="container relative">
            <?php if ($auto_accident_fault__image) { ?>
                <div class="absolute top-10 bottom-10 left-10 right-1/2 mr-8 rounded-md overflow-hidden 2xl:top-14 mdt:mr-5 md:hidden">
                    <?php echo wp_get_attachment_image($auto_accident_fault__image, 'full', '', ['class' => 'object-cover min-h-full min-w-full']) ?>
                </div>
            <?php } ?>
            <div class="flex py-10 px-6 gap-16 2xl:gap-0 md:py-5">
                ...
                <?php if ($auto_accident_fault__title || $auto_accident_fault__description) { ?>
                    <div class="w-1/2 md:w-full">
                        ...
                        <?php if ($auto_accident_fault__description) { ?>
                            <div class="text-xl mdt:text-lg xs:text-base"><?php echo $auto_accident_fault__description ?></div>
                        <?php } ?>
                    </div>
                <?php } ?>
            </div>
            <?php if ($auto_accident_fault__items) { ?>
                <div class="flex text-white py-5 px-6 rounded-md gap-16 2xl:gap-0" style="background-color: <?php echo $auto_accident_fault__block_color ?: ''  ?> ;">
                    <div class="w-1/2 md:hidden"></div>
                    <div class="w-1/2 md:w-full">
                        <?php if ($auto_accident_fault__description_repeater) { ?>
                            <p class="text-xl mb-5 xs:!text-base"><?php echo $auto_accident_fault__description_repeater ?></p>
                        <?php } ?>
                        <div class="grid grid-cols-3 gap-y-10 gap-x-3 2xl:gap-3 xs:grid-cols-2">
                            <?php foreach ($auto_accident_fault__items as $item) { ?>
                                <div class=" flex items-center gap-2 xl:flex-col">
                                    ...
                                    <?php } ?>
                                    <?php if ($item['title']) { ?>
                                        <h4 class="text-white font-third uppercase leading-snug font-bold text-xl xl:text-center mdt:!text-base"><?php echo $item['title'] ?></h4>
                                    <?php } ?>
                                </div>
                            <?php } ?>
                        </div>
