...
?>
    ...
    <div class="container py-10 lg:py-6">
        ...
        <div class="grid <?php echo count($page_select_design_two__select_pages) < 3 ? 'grid-cols-' . count($page_select_design_two__select_pages)  : 'grid-cols-4 ' ?> 2xl:!grid-cols-3 lg:!grid-cols-2 gap-[30px] ">
            <?php if (!empty($page_select_design_two__select_pages)) { ?>
                ...
                <?php foreach ($page_select_design_two__select_pages as $selected_post) { ?>
                    ...
                    ?>
                    <article class="h-full max-w-[1140px] w-full m-auto">
                        <a class="relative group h-80 w-80 flex bg-cover items-end shadow-siteWide" style="background-image: url(<?php echo $thumbnail ?: $thumbnail_placeholder ?>);" href="<?= get_permalink($selected_post) ?>">
                        </a>
